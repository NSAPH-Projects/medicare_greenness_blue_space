...
######################################################################
sub<-dataplot[dataplot$area_dec=="[0.00354,1.77]",]
...
write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_popdens_q1_res_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################
sub<-dataplot[dataplot$area_dec=="(1.77,3.15]",]
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_popdens_q2_res_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################
sub<-dataplot[dataplot$area_dec=="(3.15,4.47]",]
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_popdens_q3_res_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################
sub<-dataplot[dataplot$area_dec=="(4.47,5.87]",]
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_popdens_q4_res_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################
sub<-dataplot[dataplot$area_dec=="(5.87,7.36]",]
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_popdens_q5_res_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################
sub<-dataplot[dataplot$area_dec=="(7.36,9.15]",]
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_popdens_q6_res_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################
sub<-dataplot[dataplot$area_dec=="(9.15,11.6]",]
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_popdens_q7_res_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################
sub<-dataplot[dataplot$area_dec=="(11.6,14.9]",]
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_popdens_q8_res_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################
sub<-dataplot[dataplot$area_dec=="(14.9,21.8]",]
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_popdens_q9_res_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################
sub<-dataplot[dataplot$area_dec=="(21.8,77.5]",]
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_popdens_q10_res_urb.xlsx")
