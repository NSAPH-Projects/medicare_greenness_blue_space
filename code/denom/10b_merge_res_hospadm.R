...
res_first <- read_fst(paste0(hospdir, "first_res_20002016.fst"))
res_first$qid<-as.character(res_first$QID)
res_first$QID<-NULL
...
for (filename in list.files(old_merged)) {
  ...
  year_data <-merge(year_data,res_first, by = "qid",all.x=T,
                        suffix = c(".medicare", ".hosp_adm"))
  #remove hospitalizations from earlier years
  year_data<-as.data.table(year_data)
  nobs <- nrow(year_data)
  year_data[, res_hosp := is.na(year.hosp_adm) | year.hosp_adm >= year.medicare]
  year_data<-year_data[res_hosp!=F]
  ...
  year_data[, hosp := res==1 & year.hosp_adm == year.medicare][is.na(year.hosp_adm), hosp := FALSE][]
