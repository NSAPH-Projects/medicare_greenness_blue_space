...
dataplot$popdensity_dec<-quantcut(dataplot$area,10)
table(dataplot$popdensity_dec)

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################
sub<-dataplot[dataplot$popdensity_dec=="[0.00354,1.76]",]
...
write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_popdens_q1_cvd_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################
sub<-dataplot[dataplot$popdensity_dec=="(1.76,3.14]",]
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_popdens_q2_cvd_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################
sub<-dataplot[dataplot$popdensity_dec=="(3.14,4.45]",]
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_popdens_q3_cvd_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################
sub<-dataplot[dataplot$popdensity_dec=="(4.45,5.86]",]
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_popdens_q4_cvd_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################
sub<-dataplot[dataplot$popdensity_dec=="(5.86,7.34]",]
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_popdens_q5_cvd_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################
sub<-dataplot[dataplot$popdensity_dec=="(7.34,9.13]",]
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_popdens_q6_cvd_urb.xlsx")
...
write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_popdens_q7_cvd_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################
sub<-dataplot[dataplot$popdensity_dec=="(11.5,14.9]",]
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_popdens_q8_cvd_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################
sub<-dataplot[dataplot$popdensity_dec=="(14.9,21.7]",]
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_popdens_q9_cvd_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################
sub<-dataplot[dataplot$popdensity_dec=="(21.7,77.5]",]
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_popdens_q10_cvd_urb.xlsx")
