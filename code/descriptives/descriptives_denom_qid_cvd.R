...
merged_sharddir <- "/nfs/home/J/jok8845/shared_space/ci3_health_data/medicare/gen_admission/1999_2016/Klompmaker/cache_data/hosp_adm/cvd2/correct_followup/"
...
merged_data <-rbindlist(lapply(merged_files,
                               read_fst,
                               columns = c("qid", "sex", "race", "dual","entry_age_break.y", "statecode","followup_year"),
                               as.data.table = TRUE))
nrow(merged_data)
...
fwrite(cat_out, paste0(resultsdir, "merged_2000_2016_data_sum_cvd.csv"), row.names = T)
