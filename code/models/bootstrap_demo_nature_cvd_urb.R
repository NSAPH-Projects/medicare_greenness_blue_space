...
sub<-subset(dataplot,dataplot$sex=="1")
...
se_vi_bs$strata<-c("sex 1")
se_vi_bs$out<-c("cvd")
se_vi_bs$exposure<-c("park","ndvi_summer","occur_bs_1000m")

write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_sex1_cvd_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################

sub<-subset(dataplot,dataplot$sex=="2")
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
colnames(se_vi_bs)<-"se"
se_vi_bs$model<-c("model 5")
se_vi_bs$strata<-c("sex 2")
se_vi_bs$out<-c("cvd")
se_vi_bs$exposure<-c("park","ndvi_summer","occur_bs_1000m")

write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_sex2_cvd_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################

sub<-subset(dataplot,dataplot$race=="1")
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
colnames(se_vi_bs)<-"se"
se_vi_bs$model<-c("model 5")
se_vi_bs$strata<-c("race 1")
se_vi_bs$out<-c("cvd")
se_vi_bs$exposure<-c("park","ndvi_summer","occur_bs_1000m")

write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_race1_cvd_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################

sub<-subset(dataplot,dataplot$race=="2")
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
colnames(se_vi_bs)<-"se"
se_vi_bs$model<-c("model 5")
se_vi_bs$strata<-c("race 2")
se_vi_bs$out<-c("cvd")
se_vi_bs$exposure<-c("park","ndvi_summer","occur_bs_1000m")

write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_race2_cvd_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################

sub<-subset(dataplot,dataplot$race=="3")
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
colnames(se_vi_bs)<-"se"
se_vi_bs$model<-c("model 5")
se_vi_bs$strata<-c("race 3")
se_vi_bs$out<-c("cvd")
se_vi_bs$exposure<-c("park","ndvi_summer","occur_bs_1000m")

write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_race3_cvd_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################

sub<-subset(dataplot,dataplot$dual=="0")
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
colnames(se_vi_bs)<-"se"
se_vi_bs$model<-c("model 5")
se_vi_bs$strata<-c("dual 0")
se_vi_bs$out<-c("cvd")
se_vi_bs$exposure<-c("park","ndvi_summer","occur_bs_1000m")

write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_dual0_cvd_urb.xlsx")

######################################################################
##################### m-out-n bootstrapping ##########################
######################################################################

sub<-subset(dataplot,dataplot$dual=="1")
dataplot_boots.list<-split(sub, list(sub$zip))
num_uniq_zip <- length(unique(sub$zip))
...
colnames(se_vi_bs)<-"se"
se_vi_bs$model<-c("model 5")
se_vi_bs$strata<-c("dual 1")
se_vi_bs$out<-c("cvd")
se_vi_bs$exposure<-c("park","ndvi_summer","occur_bs_1000m")

write.csv(se_vi_bs,"/nfs/home/J/jok8845/shared_space/ci3_analysis/nature_medicare/Modeloutput/confounders/SE/se_dual1_cvd_urb.xlsx")
