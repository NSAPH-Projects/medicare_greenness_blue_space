...
admissions<-read_fst("/nfs/home/J/jok8845/shared_space/ci3_health_data/medicare/gen_admission/1999_2016/Klompmaker/admission_data/all_cvd_20002016.fst")
#40535299
##############################################################################################
...
#get unique records
admissions<-anti_join(admissions,adm2) #40535299-40244599=290700
rm(adm_duplic)
...
adm2[, length := NULL]
ndropped_missing <- ndups - nrow(adm2)
print(paste(ndropped_missing, "duplicates with missing values dropped")) #143883 dropepd
...
print(paste(ndropped_random, "duplicates dropped at random")) #735 dropped

##3.Combine with previous data
hosp_add<-rbind(admissions,adm2)

#get first hospitalization 
cvd<-hosp_add[order(ADATE), .(cvd_date = ADATE[1]), by = QID]
cvd$cvd<-c(1)
cvd[, year := year(cvd_date)]#18610833

write_fst(cvd, "/nfs/home/J/jok8845/shared_space/ci3_health_data/medicare/gen_admission/1999_2016/Klompmaker/admission_data/first_cvd2_20002016.fst")
