...
admissions<-read_fst("/nfs/home/J/jok8845/shared_space/ci3_health_data/medicare/gen_admission/1999_2016/Klompmaker/admission_data/all_adrd2_20002016.fst")
#17054164
##############################################################################################
...
#get unique records
admissions<-anti_join(admissions,adm2) #17054164-22678=17031486
rm(adm_duplic)
...
adm2[, length := NULL]
ndropped_missing <- ndups - nrow(adm2)
print(paste(ndropped_missing, "duplicates with shorter length of stay dropped")) #10604 dropepd
...
print(paste(ndropped_random, "duplicates dropped at random")) #3 dropped

##3.Combine with previous data
hosp_add<-rbind(admissions,adm2)

#get first hospitalization 
alz<-hosp_add[order(ADATE), .(alz_date = ADATE[1]), by = QID]
alz$alz<-c(1)
alz[, year := year(alz_date)] #7910111

write_fst(alz, "/nfs/home/J/jok8845/shared_space/ci3_health_data/medicare/gen_admission/1999_2016/Klompmaker/admission_data/first_adrd_20002016.fst")
